...
class StaffAPITest(AuthenticationTestCase):
    ...
    def test_mme_details(self, mock_datetime):
        ...
        mock_datetime.now.return_value = datetime(2020, 4, 27, 20, 16, 0o1)
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        response_json = response.json()
    ...
    def test_anvil_export(self, mock_zip):
        ...
        self.assertIn('\t'.join([
            'NA19675_1', 'NA19675_1', '-', '1kg project n\xe5me with uni\xe7\xf8de', '-', 'Yes', 'dbgap_stady_id_1',
